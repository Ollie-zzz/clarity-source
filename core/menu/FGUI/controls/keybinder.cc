...
void fgui::keybinder::draw() {
	...
	// bound key text size
	fgui::dimension bound_key_text_size = fgui::render.get_text_size(fgui::keybinder::get_font(), m_key > 0 ? m_iinput_system[m_key].data() : "none");

	// draw the current key
	fgui::render.text(area.left + (area.right / 2) - (bound_key_text_size.width / 2), area.top + (area.bottom / 2) - (text_size.height / 2) - 1, fgui::color(style.text.at(0)), fgui::keybinder::get_font(), m_key > 0 ? m_iinput_system[m_key].data() : "none");
}

...
void fgui::slider::draw() {
	...
	stream << std::fixed << std::setprecision(0) << std::round(m_value);
	...
	//fgui::render.text(area.left + 225, (area.top - text_size.height) - 2, fgui::color(style.text.at(0)), fgui::slider::get_font(), stream.str());
	render::draw_text_string_fgui(area.left + 215, (area.top - text_size.height), render::fonts::menu_font, stream.str(), true, fgui::color(255,255,255,255));
	// slider custom value
	if (!m_min_text.empty()) {
